...
export default function markdownItAlerts(md) {
  const alertTypes = {
    'NOTE': { 
      color: 'azure', 
  ...
  md.core.ruler.after('block', 'github-alerts', (state) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      // Look for the pattern [!TYPE] in the first paragraph
      let j = i + 1;
      while (j < tokens.length && tokens[j].type !== 'blockquote_close') {
        if (tokens[j].type === 'paragraph_open' && tokens[j + 1]?.type === 'inline') {
          ...
          if (match) {
            const type = match[0].replace(/^\[!/, '').replace(/\]\s*/, '');
            const alertConfig = alertTypes[type];
            
            if (alertConfig) {
              // Add alert class to blockquote
              tokens[i].attrSet('class', `alert alert-${type.toLowerCase()}`);
              tokens[i].attrSet('data-pico-color', alertConfig.color);
              
              // Create alert title
              const titleToken = new state.Token('html_block', '', 0);
              titleToken.content = `<div class="alert-title"><span class="alert-icon">${alertConfig.icon}</span><strong>${type}</strong></div>`;
              
              // Remove [!TYPE] from content
              tokens[j + 1].content = content.replace(match[0], '');
              
              // Insert title after paragraph_open
